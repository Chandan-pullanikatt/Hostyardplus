...
export default function PropertyCard({ property }: PropertyCardProps) {
  ...
  const imageUrl = image?.asset?._ref ? urlFor(image).width(600).height(500).url() : null

  const cardContent = (
    <div className="relative rounded-2xl overflow-hidden group cursor-pointer h-[420px]">
      {/* Image */}
      ...
      <div className="absolute top-4 left-4">
        {badge ? (
          <span className="bg-black/50 backdrop-blur-sm text-white text-xs font-sans px-3 py-1.5 rounded-full">
            {badge}
          </span>
        ) : (
          <span className="bg-black/50 backdrop-blur-sm text-white text-xs font-sans px-3 py-1.5 rounded-full">
    ...
    <div className="flex flex-col gap-3">
      ...
      {isAvailable && (
        <div className="px-1">
          ...
          <p className="font-sans text-sm text-gray-500 mt-1 line-clamp-2">{description}</p>
        </div>
      )}
    </div>
  )
